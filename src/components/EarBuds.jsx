...
const PrevArrow = (props) => {
  ...
  return (
    <div
      className={`${className} text-black text-[30px] bg-red 0 rounded-full absolute top-1/2 transform -translate-y-1/2 left-0 z-10 cursor-pointer`}
      style={{ ...style }}
      onClick={onClick}
    >
      <FaChevronLeft />
...
const EarBuds = () => {
  ...
  useEffect(()=>{
    async function fetchData(){
     try {
       const {data}=await axios.get("https://reliance-digital-backend-1.onrender.com/api/v1/users/products?category=earbuds")
